...
export default function ItemChat() {
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      const { data } = await supabase.from("item").select("id,title,nickname,user_id").eq("id", params.id).single();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", {
        ...
        const row = payload.new as { buyer_id: string; location: string | null; meet_at: string | null; updated_at: string } | null;
    ...
    <div className="mb-4">
      {!showMeetupForm ? (
        <button
          ...
          className="w-full flex items-center justify-between border border-emerald-200 rounded-2xl px-4 py-3 bg-emerald-50 hover:bg-emerald-100 transition-colors text-left"
        >
          ...
          <span className="text-xs text-emerald-700 font-bold shrink-0 ml-2">{hasMeetup ? "変更する" : "決める"}</span>
        </button>
      ) : (
        ...
        {COMMON_LOCATIONS.map((loc) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-colors ${
              meetupLocation === loc ? "bg-emerald-700 text-white border-emerald-700" : "bg-white text-emerald-700 border-emerald-200 hover:border-emerald-400"
            }`}
          >
            {loc}
          </button>
      ...
      <input
        ...
        className="w-full border border-stone-200 rounded-xl px-4 py-2 text-sm outline-none bg-white focus:border-emerald-600 transition-colors mb-3"
      />
      ...
      <div className="flex gap-1.5 flex-wrap mb-2">
        {QUICK_TIMES.map((qt) => (
          <button
            ...
            className="px-3 py-1.5 rounded-full text-xs font-bold border bg-white text-emerald-700 border-emerald-200 hover:border-emerald-400 transition-colors"
          >
            {qt.label}
          </button>
        ))}
      ...
      <div className="mb-3">
        <input
          ...
          className="border border-stone-200 rounded-xl px-4 py-2 text-sm outline-none bg-white focus:border-emerald-600 transition-colors"
        />
      </div>
      <div className="flex items-center justify-between gap-2">
        ...
        <button
          ...
          className="bg-emerald-700 text-white px-5 py-2 rounded-full text-sm font-bold disabled:opacity-50 hover:bg-emerald-800 transition-colors shrink-0"
        >
    ...
    <div className="border border-stone-200 rounded-2xl overflow-hidden bg-white shadow-sm flex flex-col h-[28rem]">
      ...
      <div className="flex border-t border-stone-200">
        <input
          ...
          disabled={!buyerId}
        />
        <button onClick={sendMessage} disabled={!buyerId} className="bg-emerald-700 text-white px-6 font-bold text-sm disabled:opacity-50 hover:bg-emerald-800 transition-colors">
          送信
        </button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        <button onClick={() => router.push(`/items/${item.id}`)} className="text-emerald-700 font-bold mb-6 flex items-center gap-1 hover:text-emerald-800 transition-colors">
              ...
              {conversationsLoading ? (
                ...
                conversations.map((c) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 border-b border-stone-100 last:border-b-0 hover:bg-stone-50 transition-colors ${selectedBuyerId === c.buyer_id ? "bg-emerald-50" : ""}`}
                  >
